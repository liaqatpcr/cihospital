...
class Config extends Secure_Controller
{
	...
	*/
	private function _licenses()
	{
		...
		if(file_exists('public/license/LICENSE'))
		{
	
			$license[$i]['text'] = $this->xss_clean(file_get_contents('public/license/LICENSE', NULL, NULL, 0, 2000));
		}
		else
		{
		...
		$dir = new DirectoryIterator('public/license');
		...
		foreach($dir as $fileinfo)
		{
			...
			if($fileinfo->isFile())
			{
				if($fileinfo->getExtension() == 'version')
				{
					...
					$basename = 'publi/license/' . $fileinfo->getBasename('.version');
					$license[$i]['title'] = $this->xss_clean(file_get_contents($basename . '.version', NULL, NULL, 0, 100));
						echo 'bangash'.$license[$i]['title'];die;
	...
	*/
	private function _themes()
	{
		...
		$dir = new DirectoryIterator('public/dist/bootswatch');
